...
def is_valid(s):
    ...
    for character in reversed(s):
        if character.isalpha():
            i += 1
        elif character.isdigit() and i >= 1:
            return False
    #condition: length between 2 and 6 char
    if len(s) <2 or len(s) > 6:
        return False
    #condition: first 2 character is letters
    elif not s[0:2].isalpha():
        return False
    #condiition: no punctuation
    elif not s.isalnum():
        return False
    else:
        return True
